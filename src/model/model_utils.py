...
def engine(messages, agent, num_agents=1, stop_sequences=None):
    if getattr(agent, "backend", None) == "gemini":
...
def get_agents(args, peft_path=None):
    ...
    elif args.model in ['gemini-3-flash', 'gemini-3-flash-preview']:
        from model.gemini import GeminiWrapper
        if not args.gemini_api_key and not os.getenv("GEMINI_API_KEY"):
            raise ValueError("Gemini API key not found. Set GEMINI_API_KEY or pass --gemini_api_key.")
        model_name = model_dirs.get(args.model, args.model)
        agent = GeminiWrapper(api_key=args.gemini_api_key or None, model_name=model_name)
